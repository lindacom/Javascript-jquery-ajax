...
const btnConfirm = document.getElementById('btn-confirm'); // confirm order button
const bookCheck = document.getElementById('book-check');

// not used?
let frmFname = document.getElementById('fname');
// const frmLname = document.getElementById('lname');
// const frmAddress = document.getElementById('address');
// const frmTown = document.getElementById('town');
...
function checkOrder() {
  ...
  if (sessionStorage.getItem('booking') !== null) {
    if (bookCheck) {
      bookCheck.innerHTML = `<h2>Booking details</h2>${JSON.parse(sessionStorage.getItem('booking'))}`;
    }
  }
}
...
// DELIVERY BOOKING
const btnBook = document.getElementsByClassName('btn-book');
const change = document.getElementById('btn-change-book');
...
function editButtons(booked) {
  if (booked === true) {
    // if edit booking button is clicked activate book buttons
    // and remove booking from session storage
    change.addEventListener('click', () => {
      sessionStorage.removeItem('booking');
    });
  }
...
for (let i = 0; i < btnBook.length; i++) {
  btnBook[i].onclick = function (e) {
    ...
    for (let j = 0; j < btnBook.length; j++) {
...
const buttonDelivery = document.getElementById('btn-delivery');
...
if (charcount) {
  charcount.addEventListener('keyup', () => {
...
// ORDER PAYMENT
let allAreFilled = false; // not used?
      ...
      if (i.value !== '') {
        ...
        if (frmValues.length == 4) { // if the 4 required fields are not empty enable button
          if (buttonDelivery.innerText === 'Book a delivery') {
            ...
            // used destructuring to set values
